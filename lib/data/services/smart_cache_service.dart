...
class SmartCacheService {
  ...
  static List<AnimeEntity>? getCachedAnimeList() {
    try {
      ...
      final animes = data.map((json) => AnimeEntity(
        id: json['id'],
        title: json['title'],
  ...
  static Future<String?> downloadAndSaveImage(String imageUrl, int animeId, int index) async {
    try {
  ...
  static Future<void> cacheAnimeImages(int animeId, List<String> imageUrls) async {
    try {
      print('📸 Caching ${imageUrls.length} images for anime $animeId');
      
      final localPaths = <String>[];
      for (int i = 0; i < imageUrls.length && i < 10; i++) { // Max 10 rasm
  ...
  static List<String>? getCachedAnimeImages(int animeId) {
    try {
      final cacheData = HiveService.getSetting<Map>('$_animeImagesPrefix$animeId');
      if (cacheData == null) return null;
      ...
      final List<dynamic> localPaths = cacheData['local_paths'] as List<dynamic>;
  ...
  static Future<bool> hasInternetConnection() async {
    try {
      final result = await http.get(Uri.parse('https://api.jikan.moe/v4')).timeout(Duration(seconds: 5));
      final hasInternet = result.statusCode == 200;
      print(hasInternet ? '🌐 Internet available' : '📴 No internet connection');
      return hasInternet;
    } catch (e) {
